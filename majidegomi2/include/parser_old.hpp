...
namespace chino::parser
{
  ...
  {
    /* using R = std::invoke_result_t <F, success_type <P, I>, I &>; */
    static_assert (result::Result <std::invoke_result_t <F, success_type <P, I>, I &>>);
    /* using T = result::success_type <R>; */
    /* using E = make_variant <failure_type <P, I>, result::failure_type <R>>; */
    return [p = std::move (p), f = std::move (f)] (I & input) constexpr noexcept
    {
  ...
  {
    return [p = std::move (p), min, max] (I & input) constexpr -> result::result <std::vector <success_type <P, I>>, failure_type <P, I>>
    {
  ...
  inline constexpr auto separated = [] <Parser <I> P, Parser <I> S> (P p, S s) constexpr noexcept
  {
    ...
    return [p = std::move (p), s = std::move (s)] (I & input) constexpr -> result::result <std::vector <success_type <P, I>>, E>
    {
      std::vector <success_type <P, I>> v;
      while (true)
      {
